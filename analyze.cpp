...
public:
	...
	double updateEnergy(const Rt::Finite_cells_iterator& begin, const Rt::Finite_cells_iterator& end, bool add = true, bool update = true) {
		...
		for (Rt::Finite_cells_iterator cell = begin; cell != end; ++cell) {
			...
			if (minimumEdgeLength(t) < minimum_edge_length){
				std::cout << "Short edge: " << minimumEdgeLength(t) << " in tetrahedron: " << t  << " with barycenter " << CGAL::centroid(t) <<  std::endl;
				if (update) {
					if (add) {
						forbidden++;
					}
			...
			if (circumradius(t) > maximum_circumradius){
				std::cout << "Large circumradius: " << circumradius(t) << " in tetrahedron: " << t  << " with barycenter " << CGAL::centroid(t) << std::endl;
				if (update) {
					if (add) {
						forbidden++;
					}
	...
	Rt::Vertex_handle chooseRandomVertexBetter(){
		...
		// }	
		Rt::Vertex_handle v = T.nearest_power_vertex(p);
		return v;
...
int main() {
	...
	// GD.iterate(1*pow(10,4));
	std::cout << GD.numberOfPoints() << " " << GD.numberOfActivePoints() << std::endl;
	GD.analyze();
